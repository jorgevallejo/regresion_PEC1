...
abline(lm(liver_volume ~ liver_weight, data = new_data))
legend("bottomright", paste("R-squared:", round(summary(lm(liver_volume ~ liver_weight))[["r.squared"]],2)), inset = 0.05, bty = 'n', cex = 0.8)
```
...
## 1e. Comprueba si las rectas de regresión de la primera gráfica son paralelas

Cuando queremos saber si dos rectas son paralelas, básicamente nos estamos haciendo la pregunta de si ambas rectas tienen la misma pendiente. Esto es, trabajaremos con la hipótesis nula de que la pendiente de ambas rectas es la misma:
$$
H_0: \beta_{females} = \beta_{males}
$$
