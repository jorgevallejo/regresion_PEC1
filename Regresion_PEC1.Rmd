...
Si recalculamos la estimación del modelo `lm()`:
```{r}
mod_snakes_rest <- lm(Trials ~ snake_ID + Day, data = snakes_long)
summary(mod_snakes_rest)
```

El cálculo de los parámetros no coincide con ninguna de las soluciones anteriores.

### Estimaciones de $\alpha_6$ y $\beta_4$

No están calculadas directamente en la regresión, pero como estamos trabajando con la restricción de que todas las alfas suman cero, y de que todas las betas suman cero, podemos calcular las estimaciones.

```{r}
a_6 <- 0 - sum(coef(mod_snakes_rest)[2:6])
b_4 <- 0 - sum(coef(mod_snakes_rest)[7:9])
```

$\alpha_6$ = `r round(a_6, 2)`  
$\beta_4$ = `r round(b_4, 2)`
...
Añadimos el producto de los estimadores $\hat \alpha_i$ y $\hat \beta_i$ a la matriz de datos para generar un nuevo modelo de regresión que nos permita contrastar este modelo de interacción con el modelo sin interacción (aditivo).

```{r}
# Generate the new parameter columns
alpha_hat <- rep(alfa, 4)
day_hat <- rep(beta, each=6)
lambda <- alpha_hat * day_hat

# Add new columns to the data matrix
snakes_long_int <- cbind(snakes_long, alpha_hat, day_hat, lambda)
head(snakes_long_int)

```

Para contrastar la hipótesis nula $H_0: \lambda = 0$ ajustaremos este nuevo modelo a los datos que incluyen el estimador $\hat \lambda$ de interacción y lo compararemos mediante la tabla ANOVA con el modelo más simple que no incluye el estimador de interacción.

```{r}
# Fitting data to the new model
mod_snake_int <- lm(Trials ~ snake_ID + Day + lambda, data = snakes_long_int )
sumary(mod_snake_int)
```

Es interesante ver que los coeficientes para los otros parámetros coinciden con los del modelo aditivo (en el apartado 2a). El coeficiente para $\lambda$ es pequeño y además no parece significativo (para un nivel de significatividad del 5%).

Si realizamos el contraste entre ambos modelos:
```{r}
anova(mod_snakes, mod_snake_int)
```

Efectivamente, no podemos rechazar la hipótesis nula de que $\lambda$ es cero. Por tanto concluimos que **no se puede detectar que haya interacción entre los parámetros _SnakeID_ y _Day_**.


## 3c. Realizar el contraste de Tukey con una función de R

Siguiendo las indicaciones de (https://rpubs.com/wgalvord/335035) realizaremos el contraste anterior utilizando esta vez la función `tukey.1df` del paquete `dae`.

En primer lugar usamos la función `aov` para ajustar el modelo aditivo añadiendo el término `Error` como al parámetro `SnakeId` anidado en `Day`.

```{r}
(snake_adit_Error_aov <- aov(Trials ~ Day + snake_ID + Error(Day/snake_ID)))
```

Seguidamente, ajustaremos el objeto `aov` que acabamos de obtener con la función `tukey.1df` del paquete `dae`.

```{r}
tukey_1df_snake_adit_Error_aov <-
  tukey.1df(aov.obj = snake_adit_Error_aov,
            data = snakes_long,
            error.term = 'Day:snake_ID')

tukey_1df_snake_adit_Error_aov
```

El valor `Tukey.p` es el p-valor asociado al estimador de $\lambda$, que podemos comprobar que es igual al que obtuvimos en el apartado 2b, igualmente no significativo y por el que concluimos que no debe de haber interacción entre los parámetros `Day` y `snake_ID`.
